...
## Two migration contexts; both stores live in AuthDb

Auth owns `AuthDbContext` (Auth schema) **and** Duende's `PersistedGrantDbContext` (`idsrv` schema), both re-scaffolded by `initial-migrations.ps1`. Both contexts run against **`AuthDb`**, so the standalone AppHost provisions only that database.
